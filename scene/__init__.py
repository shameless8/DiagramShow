...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=-1, shuffle=True, resolution_scales=[1.0]):
        ...
        if os.path.exists(os.path.join(args.source_path, "colmap_sparse")):
            os.system("cp -r {} {}".format(os.path.join(args.source_path, "colmap_sparse"), os.path.join(args.source_path, "sparse")))
        ...
        if args.val_as_train:
            scene_info.train_cameras.extend(scene_info.val_cameras)
            print(f'Total train images: {len(scene_info.train_cameras)}')
        ...
        for resolution_scale in resolution_scales:
            ...
            self.val_cameras[resolution_scale] = [] if scene_info.val_cameras is None else cameraList_from_camInfos(scene_info.val_cameras, resolution_scale, args)
        ...
        if self.loaded_iter:
            self.gaussians.load_ply(os.path.join(self.model_path,
                                                           ...
                                                           "point_cloud.ply"))
            self.first_iteration = self.loaded_iter
        else:
